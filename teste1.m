clear; clc;
a0 = [0.50;0.45;0.32;0.205;1e-5]; 
lb = [0.40,0.30,0.30,0.100,0   ];
ub = [0.60,0.80,0.50,0.300,5e-1];
...
[x,fval] =  fmincon(@(a) obj(a,Iexp,Vexp,v0),a0,[],[],[],[],lb,ub);
Vteo = linspace(0,Vexp(1)+0.15,50);
...
function [c,ceq] = cons(a)
    ...
    c=zeros(1,8);
    c(1) = v(1)- v0;
    c(2) = kk*exp((alfaa*v(14)*F)/R/T)*v(2)/(K(1)+v(2))*v(13)-(q/A)*(v(1)-v(2)); % Equação 3-13
    c(3) = v(3)- v(2);
    c(4)= v(4)- K(2)*v(3);
    c(5) = v(5)- v(4);
    c(6) = v(6)- K(3)*v(5);
